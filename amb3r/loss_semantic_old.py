...
def get_depth_loss(depth0, gt_depth, valid_mask, reduction='median', gt_scale=False):
    """
    depth0: predicted depth map (B, T, H, W, 1)
    gt_depth: ground truth depth map (B, T, H, W, 1)
    valid_mask: binary mask for valid pixels (B, T, H, W)
    reduction: 'median' or 'mean', how to reduce the loss
    """

    depth0 = depth0.clone()
    gt_depth = gt_depth.clone()
    if reduction == 'median':
        depth0[valid_mask <= 0.5] = float('nan')
        gt_depth[valid_mask <= 0.5] = float('nan')


        depth0_factor = torch.nanmedian(depth0.flatten(1), dim=1, keepdim=True).values
        gt_depth_factor = torch.nanmedian(gt_depth.flatten(1), 1, keepdim=True).values

    else:
        depth0[valid_mask <= 0.5] = float('nan')
        gt_depth[valid_mask <= 0.5] = float('nan')

        depth0_factor = torch.nanmean(depth0.flatten(1), dim=1, keepdim=True)
        gt_depth_factor = torch.nanmean(gt_depth.flatten(1), dim=1, keepdim=True)
    

    depth0 = depth0 / depth0_factor[..., None, None, None]
    gt_depth = gt_depth / gt_depth_factor[..., None, None, None]

    depth_loss = F.l1_loss(depth0[valid_mask], gt_depth[valid_mask], reduction='mean')

    return depth_loss


@torch.no_grad()
def get_point_loss(pts0, gt_pts, valid_mask, reduction='median'):
    """
    depth0: predicted depth map (B, T, H, W, 3)
    gt_depth: ground truth depth map (B, T, H, W, 3)
    valid_mask: binary mask for valid pixels (B, T, H, W)
    reduction: 'median' or 'mean', how to reduce the loss
    """
    ...
    gt_pts = gt_pts.clone()
    if reduction == 'median':
        pts0[valid_mask <= 0.5] = float('nan')
        gt_pts[valid_mask <= 0.5] = float('nan')

        pts_dist = pts0.norm(dim=-1)
        gt_dist = gt_pts.norm(dim=-1)

        pts0_factor = torch.nanmedian(pts_dist.flatten(1), dim=1, keepdim=True).values
        gt_pts_factor = torch.nanmedian(gt_dist.flatten(1), dim=1, keepdim=True).values



    else:
        raise NotImplementedError("Only median reduction is implemented for point loss")
    

    pts0 = pts0 / pts0_factor[..., None, None, None]
    gt_pts = gt_pts / gt_pts_factor[..., None, None, None]

    depth_loss = F.l1_loss(pts0[valid_mask], gt_pts[valid_mask], reduction='mean')
...
def compute_instance_loss(predictions, batch, weight=1.0, contrastive_loss_fn=None):
    ...
    return {
        ...
        "loss_instance_inter": inter.detach(),
    }
...
class MultitaskLoss(torch.nn.Module):
    ...
    def forward(self, predictions, batch, alpha=None, depth_conf=None, point_conf=None) -> torch.Tensor:
        ...
        if "pose_enc_list" in predictions and self.camera["weight"] > 0:
        ...
        depth_dict = self.depth.copy()
        depth_dict['conf_gt'] = depth_conf
        if alpha is not None:
            depth_dict['alpha'] = alpha
        if "depth" in predictions and self.depth["weight"] > 0:
            depth_loss_dict = compute_depth_loss(predictions, batch, **depth_dict)
        ...
        point_dict = self.point.copy()
        point_dict['conf_gt'] = point_conf
        if alpha is not None:
            point_dict['alpha'] = alpha
        # 3D point reconstruction loss - if world points are predicted
        if "world_points" in predictions:
            point_loss_dict = compute_point_loss(predictions, batch, **point_dict)
...
def compute_camera_loss(
    ...
    **kwargs
    ...
    # valid_mask = [batch_data[i]['valid_mask'] for i in range(len(batch_data))]
    # point_masks = torch.stack(valid_mask, dim=1).to(torch.float32)
    point_masks = batch_data['valid_mask']  # (B, N, H, W)
    # point_masks = batch_data['valid_masks']
    ...
    # poses_c2w = torch.stack([batch_data[i]['camera_pose'] for i in range(len(batch_data))], dim=1)
    # gt_extrinsics = torch.linalg.inv(poses_c2w)                                        # (B, 4, 4)
    gt_extrinsics = batch_data['extrinsics']  # (B, 4, 4)
    gt_intrinsics = batch_data['camera_intrinsics']  # (B, 3, 3)


    image_hw = batch_data['images'].shape[-2:]

    assert min(image_hw) > 3, "Image height and width must be greater than 3"
...
def compute_point_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, conf_gt=None, valid_range=-1, **kwargs):
    """
    Compute point loss.
    ...
    loss_conf, loss_grad, loss_reg = regression_loss(pred_points, gt_points, gt_points_mask, conf=pred_points_conf, conf_gt=conf_gt,
...
def compute_depth_loss(predictions, batch, gamma=1.0, alpha=0.2, gradient_loss_fn = None, conf_gt=None, valid_range=-1, **kwargs):
    """
    Compute depth loss.
    ...
    loss_conf, loss_grad, loss_reg = regression_loss(pred_depth, gt_depth, gt_depth_mask, conf=pred_depth_conf, conf_gt=conf_gt,
                                             gradient_loss_fn=gradient_loss_fn, gamma=gamma, alpha=alpha, valid_range=valid_range)
...
def regression_loss(pred, gt, mask, conf=None, gradient_loss_fn=None, gamma=1.0, alpha=0.2, valid_range=-1, conf_gt=None):
    """
    ...
    if conf_gt is None:
        ...
        loss_conf = gamma * loss_reg * conf[mask] + alpha * (conf[mask].log().mean() - conf_gt[mask].log().mean()).abs().mean()
    ...
    # Apply additional log loss
    loss_reg += torch.abs(torch.log1p(torch.norm(gt[mask], dim=-1)) - torch.log1p(torch.norm(pred[mask], dim=-1)))
    # loss_reg += torch.norm(torch.log(gt[mask] + 1) - torch.log(pred[mask] + 1), dim=-1)
    # safe_pred = torch.clamp(pred[mask], min=0)
    # safe_gt = torch.clamp(gt[mask], min=0)
    # loss_reg += torch.norm(torch.log1p(safe_gt) - torch.log1p(safe_pred), dim=-1)
