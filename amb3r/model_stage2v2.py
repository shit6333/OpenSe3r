...
class AMB3RStage2V2(nn.Module):
    ...
    def _get_voxel_feat(self, res_geo, sem_cond_sp, ins_cond_sp, B, T, H, W):
        ...
        sem_up = F.interpolate(
            sem_cond_sp.permute(0, 3, 1, 2).float(),   # (B*T, 1024, Hp, Wp) or (B*T, 1024, Ht, Wt)
    ...
    def forward(
        ...
        if sem_voxel_map is not None and ins_voxel_map is not None:
            pts_patch = _resize_pts(query_pts, Ht, Wt)    # (B*T, Ht, Wt, 3)
            mem_sem_p, mem_ins_p, mem_mask_p = _query_voxel_maps(
                pts_patch.cpu(), sem_voxel_map, ins_voxel_map,
                device=device, dtype=dtype, N=N, Hp=Ht, Wp=Wt,
                sem_dim=self.sem_dim, ins_dim=self.ins_dim,
            )
            has_memory = True
        else:
            mem_sem_p  = torch.zeros(N, self.sem_dim, Ht, Wt, device=device, dtype=dtype)
            mem_ins_p  = torch.zeros(N, self.ins_dim, Ht, Wt, device=device, dtype=dtype)
            mem_mask_p = torch.zeros(N, 1, Ht, Wt,    device=device, dtype=dtype)
            has_memory = False

        # ── Step 3: Build conditioning tensors ────────────────────────────
        # Stage-1 semantic/instance at patch resolution  (no_grad values)
        sem_s1_p = F.adaptive_max_pool2d(
            sem_feat_s1.flatten(0, 1).float(), (Ht, Wt)
        ).to(dtype)  # (B*T, sem_dim, Ht, Wt)

        ins_s1_p = F.adaptive_max_pool2d(
            ins_feat_s1.flatten(0, 1).float(), (Ht, Wt)
        ).to(dtype)  # (B*T, ins_dim, Ht, Wt)

        # cat[mem_sem, sem_s1]: (B*T, sem_dim*2, Ht, Wt) → spatial (B*T, Ht, Wt, sem_dim*2)
        sem_cond_sp = (
            torch.cat([mem_sem_p, sem_s1_p], dim=1).permute(0, 2, 3, 1)
        )  # (B*T, Ht, Wt, 1024)

        # cat[mem_ins, ins_s1, mem_mask]: (B*T, ins_dim*2+1, Ht, Wt) → spatial
        ins_cond_sp = (
        ...
        res_s2['mem_mask']            = mem_mask_p.view(B, T, 1, Ht, Wt)
        res_s2['mem_sem_for_loss']    = mem_sem_p.view(B, T, self.sem_dim, Ht, Wt) if has_memory else None
        res_s2['mem_ins_for_loss']    = mem_ins_p.view(B, T, self.ins_dim, Ht, Wt) if has_memory else None
