...
class AMB3R(nn.Module):
    def __init__(self, device='cuda', metric_scale=True,
        ...
        lseg_ckpt = './checkpoints/demo_e200.ckpt'
    ...
    @torch.no_grad()
    def _extract_lseg_feat(self, images_01):
        """
        Extract dense LSeg/CLIP features.

        Params:
            images_01: (B, T, 3, H, W)  in [0, 1] range

        Returns:
            lseg_feat: (B, T, 512, H, W)
        """
        Bs, T, _, H_orig, W_orig = images_01.shape
        imgs_flat = images_01.flatten(0, 1)       # (B*T, 3, H, W)

        # Resize to the resolution LSeg DPT expects (384 on the long side,
        # aligned to multiples of 32)
        lseg_size = 384
        scale     = lseg_size / max(H_orig, W_orig)
        H_lseg    = int(round(H_orig * scale / 32)) * 32
        W_lseg    = int(round(W_orig * scale / 32)) * 32
        ...
        lseg_feat = self.lseg.extract_features(imgs_resized)   # (B*T, 512, h, w)
        ...
        lseg_feat = lseg_feat.view(Bs, T, *lseg_feat.shape[1:])      # (B, T, 512, H, W)
        return lseg_feat
    ...
    def run_semantic_backend(self, res, images, patch_tokens, lseg_feat=None):
        '''
        run the back-end of AMB3R given front-end results and input images.
 
        lseg_feat: (B, T, 512, H, W) | None
            傳入時 get_voxel_feat 會帶 clip conditioning，
            且 decode 會用 has_backend=True 讓 semantic_head / instance_head 被呼叫。
        '''
        voxel_feat_aligned, voxel_feat_aligned_vis, voxel_layer_list = \
            self.get_voxel_feat(res, clip_feat=lseg_feat)
        ...
        res1 = self.front_end.decode_patch_tokens_and_heads(
            ...
            semantic_feats=lseg_feat,   # None 時 frontend_semantic 會忽略
            has_backend=True,
        )
    ...
    def run_amb3r_vo(self, frames, cfg, keyframe_memory):
        ...
        # if conf average > threshold, skip backend
        ...
        res_all.append(res1)    # add backend results (res_all[1])

        # Blend frontend & backend results
        compare=False
        blend = cfg.blend
        ...
        if blend:
            ...
            # if backend results is not good, back to use frontend
            if cross_consistency > 0.04 and max_median_ratio_1 / max_median_ratio_0 > 1.25 and max_median_ratio_1 > 5.0:
                return res_all[0]
        
            # fuse frontend and backend results (confidence-weighted average)
    ...
    def run_amb3r_sem_vo(self, frames, cfg, keyframe_memory):
        '''
        AMB3R-VO interface with semantic & instance feature support.
 
        與原版 run_amb3r_vo 的差異：
          2. 移除 conf_threshold_front early return（semantic 一定要跑 backend）
          3. run_backend 直接帶 lseg_feat，一次拿到幾何 + semantic/instance feat
        '''
        res_all = []
        ...
        # ── Step 2: backend（一次拿幾何 + semantic/instance）─────────────
        res1 = self.run_semantic_backend(res, images, patch_tokens=patch_tokens, lseg_feat=lseg_feat)
        res_all.append(res1)
 
        # ── Step 3: blend（與原版完全相同）──────────────────────────────
        compare = False
        blend   = cfg.blend
        ...
        if blend:
            points0 = res_all[0][pts_key]
            conf0   = res_all[0]['world_points_conf']
            c2w0    = res_all[0]['pose'][0]
 
            points1 = res_all[-1][pts_key]
            conf1   = res_all[-1]['world_points_conf']
            c2w1    = res_all[-1]['pose'][0]
            ...
            if blend and cross_consistency < 0.04:
                ...
                c2w_blend = interpolate_poses(
                    c2w0.view(-1, 4, 4), c2w1.view(-1, 4, 4),
                    conf0.view(-1, H, W), conf1.view(-1, H, W),
                )
                c2w_blend = c2w_blend.view(Bs, T, 4, 4).to(points0.device)
 
                # blend 後的 res_new：保留幾何 + 從 res1 繼承 semantic/instance feat
                res_new = {
                    pts_key:             points_final,
                    'world_points_conf': conf_sum / 2.0,
                    'pose':              c2w_blend,
                }
                # semantic/instance feat 跟著 backend result (res1) 走
                if 'semantic_feat' in res_all[1]:
                    res_new['semantic_feat'] = res_all[1]['semantic_feat']
                    res_new['semantic_conf'] = res_all[1]['semantic_conf']
                if 'instance_feat' in res_all[1]:
                    res_new['instance_feat'] = res_all[1]['instance_feat']
                    res_new['instance_conf'] = res_all[1]['instance_conf']
 
                res_all.append(res_new)
 
        # ── Step 4: keyframe consistency selection（與原版完全相同）────────
        if keyframe_memory is None:
            return res_all[-1]
        ...
        if cross_consistency > 0.04 or compare:
            ...
            for i, res in enumerate(res_all):
                ...
                aligned_pts, info = umeyama_alignment(
                    pts_pred.reshape(-1, 3).cpu(),
                    ref_pts_masked.reshape(-1, 3).cpu(),
                )
                rel_err = torch.norm(
