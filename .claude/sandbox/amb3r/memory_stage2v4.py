...
class DetachedVoxelStore:
    ...
    Key differences from DifferentiableVoxelMap:
        ...
        - First-visit anchoring by default (ema_lambda=0.0); set ema_lambda>0 for soft EMA updates
    ...
    def __init__(self, voxel_size: float, mem_dim: int, ema_lambda: float = 0.0):
        """
        ema_lambda : EMA weight for revisited voxels.
            0.0 (default) → first-visit only (backward-compatible).
            >0            → F = (1-λ)·F_old + λ·F_new on every revisit.
        """
        self.voxel_size  = voxel_size
        self.mem_dim     = mem_dim
    ...
    def update(
        ...
        """
        Vectorized voxel update.

        ema_lambda == 0.0 (default):
            First-visit anchoring — existing voxels are never overwritten.
        ema_lambda > 0:
            EMA for revisited voxels:
                F_store = (1 - λ) · F_store + λ · F_new
            New voxels are always initialised on first visit.
        """
        from torch_scatter import scatter_mean
        ...
        feats_cpu = feats_v.detach().cpu()
        ukeys_cpu = unique_keys.cpu()

        # ── Cold store: initialise all voxels directly ────────────────────
        if self._voxel_keys.shape[0] == 0:
            order = ukeys_cpu.argsort()
            self._voxel_keys  = ukeys_cpu[order]
            self._voxel_feats = feats_cpu[order]
            for k in ukeys_cpu.tolist():
                self._voxel_key_set.add(k)
            return

        # ── Split: existing (update) vs new (append) ──────────────────────
        idx_in_store = torch.searchsorted(self._voxel_keys, ukeys_cpu)
        idx_clamped  = idx_in_store.clamp(max=self._voxel_keys.shape[0] - 1)
        existing     = (self._voxel_keys[idx_clamped] == ukeys_cpu)       # [V] bool

        if existing.any():
            if self._ema_lambda > 0.0:
                pos = idx_clamped[existing]
                lam = self._ema_lambda
                self._voxel_feats[pos] = ((1.0 - lam) * self._voxel_feats[pos]
                                          + lam * feats_cpu[existing])
            # ema_lambda == 0.0 → first-visit only: skip existing voxels

        new_mask = ~existing
        if new_mask.any():
            new_keys  = ukeys_cpu[new_mask]
            new_feats = feats_cpu[new_mask]
